...
# Versão CORRIGIDA - Parâmetros válidos para o instalador
...
# Criar diretório persistente para arquivos de instalação
...
; Configuração para instalação silenciosa

[OPTIONS]
; Ação de instalação
ACTION="Install"

; Modo silencioso (parâmetros válidos)
QUIET="True"
...
# 3. INSTALAÇÃO DO SQL SERVER (COM PARÂMETROS CORRETOS)
...
Write-Host "  Parâmetros: $($arguments -join ' ')" -ForegroundColor Gray

try {
    # Executa o instalador e captura a saída
    $process = Start-Process -FilePath $sqlInstaller `
        -ArgumentList $arguments `
        -Wait `
        -PassThru `
        -NoNewWindow `
        -RedirectStandardOutput "$tempDir\install_stdout.log" `
        -RedirectStandardError "$tempDir\install_stderr.log" `
        -ErrorAction Stop

    Write-Host "  Código de saída: $($process.ExitCode)" -ForegroundColor Gray
    ...
    if ($process.ExitCode -eq 0) {
        ...
        if (Test-Path "$tempDir\install_stderr.log") {
            $errors = Get-Content "$tempDir\install_stderr.log" -ErrorAction SilentlyContinue
            if ($errors) {
        ...
        Write-Host "     Verifique o log detalhado: $sqlLog" -ForegroundColor Yellow
        
        # Tentativa alternativa sem configuration file (fallback)
        Write-Host "`n  ⚠️  Tentando método alternativo de instalação..." -ForegroundColor Yellow
        ...
        $altProcess = Start-Process -FilePath $sqlInstaller `
            -ArgumentList $altArguments `
            -Wait `
            -PassThru `
            -NoNewWindow
            
        if ($altProcess.ExitCode -eq 0) {
            Write-Host "  ✅ SQL Server instalado com sucesso (método alternativo)!" -ForegroundColor Green
        } else {
            Write-Host "  ❌ Falha também no método alternativo. Código: $($altProcess.ExitCode)" -ForegroundColor Red
        }
    }
}
catch {
...
Write-Host "  ⏳ Aguardando serviços iniciarem (30 segundos)..." -NoNewline
Start-Sleep -Seconds 30
Write-Host " OK" -ForegroundColor Green

# Verificar se o serviço do SQL está rodando
$serviceName = "MSSQLSERVER"  # Para instância padrão
...
if ($service -and $service.Status -eq 'Running') {
    ...
    try {
        ...
        # Tenta carregar o assembly do SQL Client
        ...
        -- Conceder permissões de sysadmin
...
Write-Host "    INSTALAÇÃO CONCLUÍDA!    " -ForegroundColor White -BackgroundColor Green
